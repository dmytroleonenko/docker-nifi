...
#  sed -i "s/nifi\.ui\.banner\.text=.*/nifi.ui.banner.text=${BANNER_TEXT}/g" ${NIFI_HOME}/conf/nifi.properties
  sed -i "s/nifi\.remote\.input\.host=.*/nifi.remote.input.host=$(/sbin/ip a |awk '/10.2/ { print $2 }' | awk -F/ '{print $1}' | sed 's/\./-/g').default.pod.cluster.local/g" ${NIFI_HOME}/conf/nifi.properties
